...
export interface IShipTo {
    ...
    iataCode: string;
    saveAsNewLocationId: boolean;
    saveAsDefaultShipToLocation: boolean;
}
...
const ShipTo: React.FC<{ index: number }> = ({ index }) => {
    const { values, setFieldValue, touched, errors } =
        useFormikContext<CreatePackageFormValues>();

    const shipTo = values.packages[index].shipTo;
    const shipToTouched = touched.packages?.[index]?.shipTo as any;
    const shipToErrors = errors.packages?.[index]?.shipTo as any;

    const { data: locationsData, isLoading: isLocationLoading } =
        useGetLocationMasterQuery([]);

    const [updateDefaultToLocation, { isLoading: defaultLocationLoading }] =
        useUpdateShipToDefaultLocationIdMutation();

    const allLocations = locationsData?.locations || [];

    const defaultLocationData = allLocations.find(
        (loc: Location) => loc.def_ship_to === 1
    );
    ...
    const [searchKey, setSearchKey] = useState(
        shipTo.locationId || defaultLocationDisplay
    );
    const [showSuggestions, setShowSuggestions] = useState(false);
    const [showMore, setShowMore] = useState(false);
    ...
    useEffect(() => {
        if (!defaultLocationData || shipTo.locationId) return;

        const p = `packages.${index}.shipTo`;
        ...
        setFieldValue(
            `${p}.saveAsDefaultShipToLocation`,
            defaultLocationData.def_ship_to || false
        );
    }, [defaultLocationData, index, setFieldValue, shipTo.locationId]);
        ...
        <Grid>
            ...
            <Grid container spacing={2} className={styles.formsBgContainer}>
                ...
                <FormControlLabel
                    control={
                        <Field
                            name={`packages.${index}.shipTo.saveAsDefaultShipToLocation`}
                            type="checkbox"
                            as={Checkbox}
                        />
                    }
                ...
                <Grid item xs={12} md={4}>
                    <TextField
                        ...
                        error={
                            shipToTouched?.locationId &&
                            Boolean(shipToErrors?.locationId)
                        }
                        helperText={
                            shipToTouched?.locationId && shipToErrors?.locationId
                        }
                        InputProps={{
                        ...
                        <Paper sx={{ maxHeight: 200, overflowY: 'auto', mt: 1 }}>
                            <List>
                                {displayLocations.map((loc: Location) => (
                                    ...
                                    >
                                        {loc.loc_ID}, {loc.loc_desc}, {loc.city}, {loc.state},{' '}
                                        {loc.pincode}
                                    </ListItem>
                                ))}
                            </List>
                ...
                <Grid item xs={12} md={4}>
                    <Field
                        name={`packages.${index}.shipTo.locationDescription`}
                        as={TextField}
                        label="Location Description*"
                        size="small"
                        fullWidth
                ...
                <Collapse in={showMore}>
                    <Grid container spacing={2} mt={1}>
                        <Grid item xs={12} md={3}>
                            <Field
                                name={`packages.${index}.shipTo.addressLine1`}
                                as={TextField}
                                label="Address Line 1*"
                                size="small"
                                fullWidth
                        ...
                        <Grid item xs={12} md={3}>
                            <Field
                                name={`packages.${index}.shipTo.addressLine2`}
                                as={TextField}
                                label="Address Line 2"
                                size="small"
                                fullWidth
                        ...
                        <Grid item xs={12} md={3}>
                            <Field
                                name={`packages.${index}.shipTo.city`}
                                as={TextField}
                                label="City*"
                                size="small"
                                fullWidth
                        ...
                        <Grid item xs={12} md={3}>
                            <Field
                                name={`packages.${index}.shipTo.state`}
                                as={TextField}
                                label="State*"
                                size="small"
                                fullWidth
                        ...
                        <Grid item xs={12} md={3}>
                            <Field
                                name={`packages.${index}.shipTo.country`}
                                as={TextField}
                                label="Country*"
                                size="small"
                                fullWidth
                        ...
                        <Grid item xs={12} md={3}>
                            <Field
                                name={`packages.${index}.shipTo.pincode`}
                                as={TextField}
                                label="Pincode*"
                                size="small"
                                fullWidth
                        ...
                        <Grid item xs={12} md={3}>
                            <Field
                                name={`packages.${index}.shipTo.contactPerson`}
                                as={TextField}
                                label="Contact Person"
                                size="small"
                                fullWidth
                        ...
                        <Grid item xs={12} md={3}>
                            <Field
                                name={`packages.${index}.shipTo.phoneNumber`}
                                as={TextField}
                                label="Phone Number"
                                size="small"
                                fullWidth
                        ...
                        <Grid item xs={12} md={3}>
                            <Field
                                name={`packages.${index}.shipTo.email`}
                                as={TextField}
                                label="Email"
                                size="small"
                                fullWidth
                        ...
                        <Grid item xs={12} md={3}>
                            <Field
                                name={`packages.${index}.shipTo.glnCode`}
                                as={TextField}
                                label="GLN Code"
                                size="small"
                                fullWidth
                        ...
                        <Grid item xs={12} md={3}>
                            <Field
                                name={`packages.${index}.shipTo.iataCode`}
                                as={TextField}
                                label="IATA Code"
                                size="small"
                                fullWidth
