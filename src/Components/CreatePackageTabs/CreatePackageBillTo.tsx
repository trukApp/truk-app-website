...
export interface IBillTo {
    ...
    iataCode: string;
    saveAsNewLocationId: boolean;
    saveAsDefaultBillToLocation: boolean;
}
...
const BillTo: React.FC<{ index: number }> = ({ index }) => {
    const { values, setFieldValue, touched, errors } = useFormikContext<CreatePackageFormValues>();

    const billTo = values.packages[index].billTo;
    const billToTouched = touched.packages?.[index]?.billTo as any;
    const billToErrors = errors.packages?.[index]?.billTo as any;

    const { data: locationsData, isLoading: isLocationLoading } = useGetLocationMasterQuery([]);
    const [updateDefaultLocation, { isLoading: defaultLocationLoading }] = useUpdateBillToDefaultLocationIdMutation();

    const allLocations = locationsData?.locations || [];
    const defaultLocationData = allLocations.find((loc: Location) => loc.def_bill_to === 1);

    const defaultLocationDisplay = defaultLocationData
        ? `${defaultLocationData.loc_ID}, ${defaultLocationData.loc_desc}, ${defaultLocationData.city}, ${defaultLocationData.state}, ${defaultLocationData.pincode}`
        : '';

    const [searchKey, setSearchKey] = useState(billTo.locationId || defaultLocationDisplay);
    const [showSuggestions, setShowSuggestions] = useState(false);
    const [showMore, setShowMore] = useState(false);
    ...
    useEffect(() => {
        if (!defaultLocationData || billTo.locationId) return;

        const p = `packages.${index}.billTo`;

        setFieldValue(`${p}.locationId`, defaultLocationData.loc_ID || '');
        setFieldValue(`${p}.locationDescription`, defaultLocationData.loc_desc || '');
        setFieldValue(`${p}.addressLine1`, defaultLocationData.address_1 || '');
        setFieldValue(`${p}.addressLine2`, defaultLocationData.address_2 || '');
        setFieldValue(`${p}.city`, defaultLocationData.city || '');
        setFieldValue(`${p}.state`, defaultLocationData.state || '');
        setFieldValue(`${p}.country`, defaultLocationData.country || '');
        setFieldValue(`${p}.pincode`, defaultLocationData.pincode || '');
        setFieldValue(`${p}.latitude`, defaultLocationData.latitude || '');
        setFieldValue(`${p}.longitude`, defaultLocationData.longitude || '');
        setFieldValue(`${p}.timeZone`, defaultLocationData.time_zone || '');
        setFieldValue(`${p}.locationType`, defaultLocationData.loc_type || '');
        setFieldValue(`${p}.glnCode`, defaultLocationData.gln_code || '');
        setFieldValue(`${p}.iataCode`, defaultLocationData.iata_code || '');
        setFieldValue(`${p}.contactPerson`, defaultLocationData.contact_name || '');
        setFieldValue(`${p}.phoneNumber`, defaultLocationData.contact_phone_number || '');
        setFieldValue(`${p}.email`, defaultLocationData.contact_email || '');
        setFieldValue(`${p}.saveAsDefaultBillToLocation`, defaultLocationData.def_bill_to || false);
    }, [defaultLocationData, index, setFieldValue, billTo.locationId]);

    /* ================= HANDLERS ================= */
    const handleLocationSelect = (location: Location) => {
        const p = `packages.${index}.billTo`;

        setSearchKey(`${location.loc_ID}, ${location.loc_desc}, ${location.city}, ${location.state}, ${location.pincode}`);
        setShowSuggestions(false);

        setFieldValue(`${p}.locationId`, location.loc_ID);
        setFieldValue(`${p}.locationDescription`, location.loc_desc || '');
        setFieldValue(`${p}.addressLine1`, location.address_1 || '');
        setFieldValue(`${p}.addressLine2`, location.address_2 || '');
        setFieldValue(`${p}.city`, location.city || '');
        setFieldValue(`${p}.state`, location.state || '');
        setFieldValue(`${p}.country`, location.country || '');
        setFieldValue(`${p}.pincode`, location.pincode || '');
        setFieldValue(`${p}.latitude`, location.latitude || '');
        setFieldValue(`${p}.longitude`, location.longitude || '');
        setFieldValue(`${p}.timeZone`, location.time_zone || '');
        setFieldValue(`${p}.locationType`, location.loc_type || '');
        setFieldValue(`${p}.glnCode`, location.gln_code || '');
        setFieldValue(`${p}.iataCode`, location.iata_code || '');
        setFieldValue(`${p}.contactPerson`, location.contact_name || '');
        setFieldValue(`${p}.phoneNumber`, location.contact_phone_number || '');
        setFieldValue(`${p}.email`, location.contact_email || '');
    };

    const handleDefaultLocationChange = async (locId: string, defaultValue: number | boolean) => {
        try {
            const updatedLocationId = locId?.split(',')[0] ?? '';
            await updateDefaultLocation({ locId: updatedLocationId, defShipTo: defaultValue ? 1 : 0 }).unwrap();
        } catch (error) {
            console.error('Error updating default Bill To location: ', error);
        }
    };
    ...
    return (
        <Grid>
            <Backdrop sx={{ color: '#fff', zIndex: 1300 }} open={isLocationLoading || defaultLocationLoading}>
                <CircularProgress />
            </Backdrop>
            ...
            <Grid container spacing={2} className={styles.formsBgContainer}>
                ...
                <FormControlLabel
                    control={
                        <Field
                            name={`packages.${index}.billTo.saveAsDefaultBillToLocation`}
                            type="checkbox"
                            as={Checkbox}
                        />
                    }
                ...
                <Grid item xs={12} md={4}>
                    <TextField
                        ...
                        error={billToTouched?.locationId && Boolean(billToErrors?.locationId)}
                        helperText={billToTouched?.locationId ? billToErrors?.locationId : ''}
                        InputProps={{
                            endAdornment: filteredLocationLoading ? <CircularProgress size={18} /> : null,
                        }}
                    />
                        ...
                        <Paper sx={{ maxHeight: 200, overflowY: 'auto', mt: 1 }}>
                            <List>
                                {displayLocations.map((loc: Location) => (
                                    <ListItem key={loc.loc_ID} sx={{ cursor: 'pointer' }} onClick={() => handleLocationSelect(loc)}>
                                        {loc.loc_ID}, {loc.loc_desc}, {loc.city}, {loc.state}, {loc.pincode}
                                    </ListItem>
                                ))}
                            </List>
                        </Paper>
                ...
                <Grid item xs={12} md={4}>
                    <Field
                        name={`packages.${index}.billTo.locationDescription`}
                        as={TextField}
                        label="Location Description*"
                        size="small"
                        fullWidth
                ...
                <Grid item xs={12} md={4}>
                    <Typography sx={{ mt: 1, cursor: 'pointer', fontWeight: 'bold' }} onClick={() => setShowMore(!showMore)}>
                        {showMore ? 'Hide Details ↑' : 'More Details ↓'}
                    </Typography>
                </Grid>
                ...
                <Collapse in={showMore}>
                    <Grid container spacing={2} mt={1}>
                        {[
                            ...
                            { name: 'glnCode', label: 'GLN Code' },
                            { name: 'iataCode', label: 'IATA Code' },
                        ].map((field) => (
                            <Grid item xs={12} md={3} key={field.name}>
                                <Field
                                    name={`packages.${index}.billTo.${field.name}`}
                                    as={TextField}
                                    label={field.label}
                                    size="small"
                                    fullWidth
