...
export const createPackageValidationSchema = Yup.object({
  ...
  shipTo: locationInfoSchema
    .concat(
      Yup.object({
        destination_radius: Yup.string().required(
          "Destination radius is required",
        ),
        destination_radius_unit: Yup.string().required(
          "Destination radius unit is required",
        ),
      }),
    )
  ...
  pickupDropoff: pickupDropoffSchema.required(
    "Pickup and dropoff details are required",
  ),
});
