...
const ShipFrom: React.FC<{ index: number }> = ({ index }) => {
    const { values, setFieldValue, touched, errors } =
        useFormikContext<CreatePackageFormValues>();

    const shipFrom = values.packages[index].shipFrom;
    const shipFromTouched = touched.packages?.[index]?.shipFrom as any;
    const shipFromErrors = errors.packages?.[index]?.shipFrom as any;

    const { data: locationsData, isLoading: isLocationLoading } =
        useGetLocationMasterQuery([]);

    const [updateDefaultFromLocation, { isLoading: defaultLocationLoading }] =
        useUpdateShipFromDefaultLocationIdMutation();

    const allLocations = locationsData?.locations || [];

    const defaultLocationData = allLocations.find(
        (loc: Location) => loc.def_ship_from === 1
    );
    ...
    const [searchKey, setSearchKey] = useState(
        shipFrom.locationId || defaultLocationDisplay
    );
    const [showSuggestions, setShowSuggestions] = useState(false);
    const [showMore, setShowMore] = useState(false);
    ...
    useEffect(() => {
        if (!defaultLocationData || shipFrom.locationId) return;

        const p = `packages.${index}.shipFrom`;
        ...
        setFieldValue(
            `${p}.phoneNumber`,
            defaultLocationData.contact_phone_number || ''
        );
        setFieldValue(`${p}.email`, defaultLocationData.contact_email || '');
        setFieldValue(
            `${p}.saveAsDefaultShipFromLocation`,
            defaultLocationData.def_ship_from || false
        );
    }, [defaultLocationData, index, setFieldValue, shipFrom.locationId]);
        ...
        <Grid>
            ...
            <Grid container spacing={2} className={styles.formsBgContainer}>
                ...
                <FormControlLabel
                    control={
                        <Field
                            name={`packages.${index}.shipFrom.saveAsDefaultShipFromLocation`}
                            type="checkbox"
                            as={Checkbox}
                        />
                    }
                ...
                <Grid item xs={12} md={4}>
                    <TextField
                        ...
                        error={
                            shipFromTouched?.locationId &&
                            Boolean(shipFromErrors?.locationId)
                        }
                        helperText={
                            shipFromTouched?.locationId && shipFromErrors?.locationId
                        }
                        InputProps={{
                        ...
                        <Paper sx={{ maxHeight: 200, overflowY: 'auto', mt: 1 }}>
                            <List>
                                {displayLocations.map((loc: Location) => (
                                    ...
                                    >
                                        {loc.loc_ID}, {loc.loc_desc}, {loc.city},{' '}
                ...
                <Grid item xs={12} md={4}>
                    <Field
                        name={`packages.${index}.shipFrom.locationDescription`}
                        as={TextField}
                        label="Location Description*"
                        size="small"
                        fullWidth
                ...
                <Collapse in={showMore}>
                    <Grid container spacing={2} mt={1}>
                        <Grid item xs={12} md={3}>
                            <Field
                                name={`packages.${index}.shipFrom.addressLine1`}
                                as={TextField}
                                label="Address Line 1*"
                                size="small"
                                fullWidth
                        ...
                        <Grid item xs={12} md={3}>
                            <Field
                                name={`packages.${index}.shipFrom.addressLine2`}
                                as={TextField}
                                label="Address Line 2"
                                size="small"
                                fullWidth
                        ...
                        <Grid item xs={12} md={3}>
                            <Field
                                name={`packages.${index}.shipFrom.city`}
                                as={TextField}
                                label="City*"
                                size="small"
                                fullWidth
                        ...
                        <Grid item xs={12} md={3}>
                            <Field
                                name={`packages.${index}.shipFrom.state`}
                                as={TextField}
                                label="State*"
                                size="small"
                                fullWidth
                        ...
                        <Grid item xs={12} md={3}>
                            <Field
                                name={`packages.${index}.shipFrom.country`}
                                as={TextField}
                                label="Country*"
                                size="small"
                                fullWidth
                        ...
                        <Grid item xs={12} md={3}>
                            <Field
                                name={`packages.${index}.shipFrom.pincode`}
                                as={TextField}
                                label="Pincode*"
                                size="small"
                                fullWidth
                        ...
                        <Grid item xs={12} md={3}>
                            <Field
                                name={`packages.${index}.shipFrom.contactPerson`}
                                as={TextField}
                                label="Contact Person"
                                size="small"
                                fullWidth
                        ...
                        <Grid item xs={12} md={3}>
                            <Field
                                name={`packages.${index}.shipFrom.phoneNumber`}
                                as={TextField}
                                label="Phone Number"
                                size="small"
                                fullWidth
                        ...
                        <Grid item xs={12} md={3}>
                            <Field
                                name={`packages.${index}.shipFrom.email`}
                                as={TextField}
                                label="Email"
                                size="small"
                                fullWidth
                        ...
                        <Grid item xs={12} md={3}>
                            <Field
                                name={`packages.${index}.shipFrom.glnCode`}
                                as={TextField}
                                label="GLN Code"
                                size="small"
                                fullWidth
                        ...
                        <Grid item xs={12} md={3}>
                            <Field
                                name={`packages.${index}.shipFrom.iataCode`}
                                as={TextField}
                                label="IATA Code"
                                size="small"
                                fullWidth
