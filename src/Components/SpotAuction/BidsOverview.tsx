...
const BidsOverview: React.FC<BidsOverviewProps> = ({
  bidsData,
  carriers,
  orderId,
}) => {
  ...
  const handleEditAuction = async (values: {
    bid_value: string;
    ...
    const editAuctionBody: {
      bid_value?: string;
      bid_closing_time: string | null;
    } = {
  ...
  return (
    <Paper sx={{ p: 3, mb: 3, borderRadius: 3, boxShadow: 3 }}>
      ...
      {bidsData.map((bid: Bid, index: number) => {
        ...
        const displayCarrier = displayBid
          ? getCarrierDetails(displayBid.bid_from)
          : null;
            ...
            >
              ...
              <Grid sx={{ flex: 1 }}>
                ...
                <Typography variant="body2">
                  <strong>Bid ID:</strong> {bid.bid_id}
                </Typography>
                <Typography variant="body2">
                  <strong>Order ID:</strong> {bid.order_ID}
                </Typography>
                <Typography variant="body2">
                  <strong>Bid Value:</strong> ₹{bid.bid_value}
                </Typography>
                <Typography variant="body2">
                  <strong>Start Time:</strong>{" "}
                  {moment(bid.bid_start_time).format("DD MMM YYYY HH:mm")}
                </Typography>
                <Typography variant="body2">
                  <strong>End Time:</strong>{" "}
                  {moment(bid.bid_closing_time).format("DD MMM YYYY HH:mm")}
                </Typography>
              </Grid>
              ...
              {displayBid && (
                ...
                >
                  ...
                  <Grid sx={{ mt: 3 }}>
                    ...
                    >
                      <strong>Bid From:</strong> {displayBid.bid_from} →{" "}
                      <strong>₹{displayBid.bid_amount}</strong>
                    </Typography>
                    <Typography
                      variant="body2"
                      sx={{ mb: 0.5, fontSize: "0.8rem" }}
                    >
                      ...
                      {moment(displayBid.bid_placed_at).format(
                        "DD MMM YYYY HH:mm",
                      )}
                    </Typography>
                    ...
                    {displayCarrier ? (
                      <Grid>
                        <Typography variant="body2" sx={{ fontSize: "0.8rem" }}>
                          <strong>Name:</strong>{" "}
                          {displayCarrier.carrier_name || "-"}
                        </Typography>
                        <Typography variant="body2" sx={{ fontSize: "0.8rem" }}>
                          <strong>Phone:</strong>{" "}
                          {displayCarrier.carrier_correspondence?.phone || "-"}
                        </Typography>
                        <Typography variant="body2" sx={{ fontSize: "0.8rem" }}>
                          <strong>Email:</strong>{" "}
                          {displayCarrier.carrier_correspondence?.email || "-"}
                        </Typography>
                      </Grid>
                    ) : (
                      <Typography
                      ...
                      <Grid item key={i}>
                        ...
                        >
                          <Typography variant="body2" sx={{ mb: 1 }}>
                            <strong>Bid From:</strong> {ab.bid_from} →{" "}
                            <strong>₹{ab.bid_amount}</strong>
                          </Typography>
                          <Typography variant="body2" sx={{ mb: 1 }}>
                            <strong>Placed At:</strong>{" "}
                            {moment(ab.bid_placed_at).format(
                              "DD MMM YYYY HH:mm",
                            )}
                          </Typography>
                          ...
                          {carrier ? (
                            <Grid>
                              ...
                              <Typography variant="body2">
                                <strong>Phone:</strong>{" "}
                                {carrier.carrier_correspondence?.phone || "-"}
                              </Typography>
                              <Typography variant="body2">
                                <strong>Email:</strong>{" "}
                                {carrier.carrier_correspondence?.email || "-"}
                              </Typography>
                            </Grid>
                          ) : (
                            <Typography
          ...
          {selectedBid && (
            <Formik<{
              ...
              initialValues={{
                ...
                bid_end_time: selectedBid.bid_end_time
                  ? moment(selectedBid.bid_end_time)
                  : null,
                        ...
                        <DateTimePicker
                          ...
                          slotProps={{
                            textField: {
                              ...
                              margin: "normal",
                              error: Boolean(
                                touched.bid_start_time && errors.bid_start_time,
                              ),
                              helperText:
                                touched.bid_start_time && errors.bid_start_time,
                            },
                    ...
                    <Field name="bid_end_time">
                      {({ field }: FieldProps<Moment | null>) => (
                        <DateTimePicker
                          ...
                          // value={field.value}
                          value={
                            field.value || moment(selectedBid.bid_closing_time)
                          }
