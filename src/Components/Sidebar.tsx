...
const Sidebar = () => {
  ...
  const renderMenu = ({
    ...
    <>
      <Tooltip title={collapsed ? title : ""} placement="right">
        <ListItemButton onClick={() => toggleMenu(keyName)} sx={menuStyles}>
          <ListItemIcon
            sx={{
              color,
          ...
          {!collapsed && (
            <>
              ...
              {openMenus[keyName] ? <ExpandLess /> : <ExpandMore />}
            </>
          )}
        </ListItemButton>
      </Tooltip>

      <Collapse in={collapsed ? false : openMenus[keyName]}>
  ...
  return (
    ...
    >
      ...
      >
        {!collapsed && (
          <Typography fontSize={18} fontWeight={700} color="#F08C24">
            CONTROL TOWER
          </Typography>
        )}
