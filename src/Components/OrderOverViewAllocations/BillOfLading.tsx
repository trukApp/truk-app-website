...
const BillOfLading: React.FC<AllocationsProps> = ({
  allocations,
  orderId,
      ...
      {allocations.map((allocation, allocIndex) => {
        ...
        return Object.entries(packagesByShipTo).map(
          ([shipToId, pkgList], index) => {
            ...
            const shipperDetails = getCustomerDetails(
              shipperPkg?.ship_from ?? "",
            );
            ...
            const filteredPackageInfo = allocation.packageInfoDetails?.filter(
              (pkg) => pkgList.some((p) => p.pack_ID === pkg.pkg_ID),
            );

            return (
              <Grid key={`${allocIndex}-${index}`} mt={4}>
                <Grid textAlign="right" mb={1}>
                  {existingBOL ? (
                    <Button
                      variant="outlined"
                      color="secondary"
                      ...
                      onClick={() =>
                        existingBOL &&
                        window.open(existingBOL.self_bill_url, "_blank")
                      }
                    >
                      ⬇ Download BOL
                    </Button>
                ...
                >
                  ...
                  <Grid container spacing={2}>
                    <Grid item xs={4}>
                      <Typography>
                        Sales Order:{" "}
                        <strong>
                          {shipperPkg?.additional_info?.sales_order_number ||
                            "-"}
                        </strong>
                      </Typography>
                    </Grid>
                    <Grid item xs={4}>
                      <Typography>
                        Cust. PO No:{" "}
                        <strong>
                          {shipperPkg?.additional_info?.po_number || "-"}
                        </strong>
                      </Typography>
                    </Grid>
                    <Grid item xs={4}>
                      <Typography>
                        Delivery: <strong>{shipperPkg?.pac_id || "-"}</strong>
                      </Typography>
                    </Grid>
                  </Grid>
                  ...
                  <Grid container spacing={2} sx={{ mt: 1 }}>
                    <Grid item xs={4}>
                      ...
                      <Typography>
                        {getLocationDetails(shipperPkg?.ship_from)}
                      </Typography>
                    </Grid>
                    <Grid item xs={4}>
                    ...
                    <Grid item xs={4}>
                      ...
                      <Typography>
                        {billToDetails
                          ? getLocationDetails(shipperPkg?.bill_to)
                          : "—"}
                      </Typography>
                    </Grid>
                  </Grid>
                        ...
                        pkg.lines.map((line, j) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <Grid item xs={3}>
                                {line.package_info?.handling_unit_type ||
                                  line.package_info?.packaging_type_name ||
                                  "-"}
                              </Grid>
                              <Grid item xs={2}>
                                {line.quantity}
                  ...
                  {/* Declaration */}
                  <Grid mt={2} sx={{ border: "1px solid #000", p: 2 }}>
                    <Typography
                      sx={{ fontWeight: "bold", textAlign: "center", mb: 1 }}
                    >
                      DRIVE WILL NOT REIMBURSE CARRIER FOR CONSIGNEE ADDED
                      ACCESSORIAL CHARGES WITHOUT PRIOR CONSENT
                    </Typography>
                    <Typography sx={{ fontSize: "11px", mb: 2 }}>
                      This is to certify that the here-in named materials are
                      properly classified, described, packaged, marked, and
                      labelled, and are in proper condition for transportation
                    ...
                    <Grid container spacing={2} sx={{ mb: 2 }}>
                      <Grid item xs={6}>
                        <Typography>
                          Shipper Signature: ___________________________
                        </Typography>
                      </Grid>
                      <Grid item xs={6}>
                        <Typography>
                    ...
                    <Grid container spacing={2} sx={{ mb: 2 }}>
                      ...
                      <Grid item xs={6}>
                        <Typography>
                          # of Pallets: ______ &nbsp;&nbsp; # of Cartons: ______
                        </Typography>
                      </Grid>
                    </Grid>
                    <Grid container spacing={2} sx={{ mb: 2 }}>
                      <Grid item xs={8}>
                        <Typography>
                          All items received in good condition: [ ] Yes
                          &nbsp;&nbsp; [ ] No
                        </Typography>
                      </Grid>
                      <Grid item xs={4}>
