...
const Allocations: React.FC<AllocationsProps> = ({
  ...
  const vehiclesData =
    allVehicleTrucks?.data.length > 0 ? allVehicleTrucks?.data : [];
  const { data: filteredVehicles, isLoading: filteredVehicleLoading } =
    useGetFilteredVehiclesQuery(
      searchKeyVehicle.length >= 3 ? searchKeyVehicle : null,
      { skip: searchKeyVehicle.length < 3 },
    );
  ...
  const [postInitiateBidding, { isLoading: isBidding }] =
    usePostInitiateBiddingMutation();
  const [editAssignOrder, { isLoading: editAssignLoading }] =
    useEditAssignOrderOrderMutation();
  const [postAssignCarrier, { isLoading: carrierAssinLoading }] =
    usePostAssignCarrierMutation();
  const [postAssignCarrierToOrder, { isLoading: carrierToOrderLoading }] =
    usePostAssignCarrierToOrderMutation();
  const { data: assignedOrder } = useGetAssignedOrderByIdQuery({
    order_ID: orderId,
  });
  ...
  useEffect(() => {
    if (
      (order?.order?.order_status === "carrier assignment" ||
        order?.order?.order_status === "carrier confirmed" ||
        order?.order?.order_status === "carrier rejected") &&
  ...
  const handleCarrierAssign = async () => {
    ...
    try {
      ...
      if (
        response?.message === "Carrier assignment initialized successfully."
      ) {
        const assignedCarrier = response?.req_sent_to[0];
        setMultipleCarriers(false);
        setSnackbarMessage(
          `Carrier assignment sent to contracted carrier ${assignedCarrier} successfully!`,
        );
        setSnackbarSeverity("success");
        setSnackbarOpen(true);
      }
  ...
  const handleBidSubmit = async (values: BiddingModalProps) => {
    const formattedStartDate = moment(values.bid_start_time).format(
      "YYYY-MM-DDTHH:mm:ss",
    );
    const formattedEndDate = moment(values.bid_end_time).format(
      "YYYY-MM-DDTHH:mm:ss",
    );
                        ...
                        <Grid container spacing={2}>
                          <Grid item xs={12} sm={6} md={2.4}>
                            <FormControl
                              ...
                              size="small"
                              error={
                                touched.vehicleId && Boolean(errors.vehicleId)
                              }
                            >
                        ...
                        <Grid container spacing={2}>
                          <Grid item xs={12} sm={6} md={2.4}>
                            <TextField
                              ...
                              onBlur={handleBlur}
                              error={
                                touched.insurance && Boolean(errors.insurance)
                              }
                          ...
                          <Grid item xs={12} sm={6} md={2.4}>
                            <TextField
                              ...
                              onChange={handleChange}
                              onBlur={handleBlur}
                              error={touched.permit && Boolean(errors.permit)}
                              helperText={touched.permit && errors.permit}
                              size="small"
                            />
                          </Grid>
                        </Grid>
                  ...
                  <Autocomplete
                    ...
                    filterOptions={(options, state) => {
                      ...
                      return options.filter((driver: any) => {
                        ...
                        const locationMatch = driver.locations?.some(
                          (loc: string) => loc.toLowerCase().includes(search),
                        );

                        return (
                          nameMatch || idMatch || phoneMatch || locationMatch
                        );
                      });
                    }}
                  />
