...
const SupplierForm: React.FC = () => {
  ...
  const [updatePartnerDetails, { isLoading: postVendorLoading }] =
    useEditBusinessPartnerMutation();
  const [customerRegistration, { isLoading: editVendorLoading }] =
    useCustomerRegistrationMutation();
  const [deleteBusinessPartner, { isLoading: deleteVendorLoading }] =
    useDeleteBusinessPartnerMutation();
  ...
  const handleDelete = async (rowData: Customer) => {
    ...
    try {
      ...
      if (response.data.deleted_record) {
        setSnackbarMessage(
          `Supplier ID ${response.data.deleted_record} deleted successfully!`,
        );
  ...
  const handleSupplierSubmit = async (
    values: typeof initialSupplierValues,
    ...
    try {
      ...
      if (updateRecord) {
        ...
        if (response?.updated_record) {
          setSnackbarMessage(
            `Supplier ID ${response.updated_record} updated successfully!`,
          );
        ...
        if (response?.created_records) {
          setSnackbarMessage(
            `Supplier ID ${response.created_records[0]} created successfully!`,
          );
          setFormInitialValues(initialSupplierValues);
          setShowForm(false);
          setUpdateRecord(false);
                ...
                <Grid container spacing={2} style={{ marginBottom: "30px" }}>
                  <Grid item xs={12} sm={6} md={2.4}>
                    <TextField
                      ...
                      onBlur={handleBlur}
                      error={
                        touched.orderingAddress &&
                        Boolean(errors.orderingAddress)
                      }
                      helperText={
                        touched.orderingAddress && errors.orderingAddress
                      }
                    />
                  </Grid>
                  <Grid item xs={12} sm={6} md={2.4}>
                    <TextField
                      ...
                      onBlur={handleBlur}
                      error={
                        touched.goodsSupplier && Boolean(errors.goodsSupplier)
                      }
                  ...
                  <Grid item xs={12} sm={6} md={2.4}>
                    <TextField
                      ...
                      onBlur={handleBlur}
                      error={
                        touched.forwardingAgent &&
                        Boolean(errors.forwardingAgent)
                      }
                      helperText={
                        touched.forwardingAgent && errors.forwardingAgent
                      }
                    />
                  </Grid>
                </Grid>
