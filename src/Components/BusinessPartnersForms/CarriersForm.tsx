...
const CarrierForm: React.FC = () => {
  ...
  const { data, isLoading } = useGetCarrierMasterQuery({
    page: paginationModel.page + 1,
    limit: paginationModel.pageSize,
  });
  const [postCarrier, { isLoading: postCarrierLoading }] =
    usePostCarrierMasterMutation();
  const [editCarrier, { isLoading: editCarrierLoading }] =
    useEditCarrierMasterMutation();
  const [deleteCarrier, { isLoading: deleteCarrierLoading }] =
    useDeleteCarrierMasterMutation();
  ...
  const handleDelete = async (row: CarrierFormFE) => {
    ...
    const confirmed = window.confirm(
      "Are you sure you want to delete this vehicle? This action canot be undone.",
    );
    if (!confirmed) {
      return;
    }
    ...
    try {
      ...
      if (response.data.deleted_record) {
        setSnackbarMessage(
          `Carrier ID ${response.data.deleted_record} deleted successfully!`,
        );
  ...
  const carrierValidationSchema = Yup.object({
    ...
    laneIds: Yup.array()
      ...
      .required("Lane IDs are required"),
  ...
  const handleCarrierSubmit = async (values: CarrierFormFE) => {
    try {
      ...
      if (isEditing && editRow) {
        ...
        const response = await editCarrier({
          body: editBody,
          carrierId,
        }).unwrap();
        if (response?.updated_record) {
          setSnackbarMessage(
            `Carrier ID ${response.updated_record} updated successfully!`,
          );
        ...
        if (response?.created_records) {
          setSnackbarMessage(
            `Carrier ID ${response.created_records[0]} created successfully!`,
          );
          setInitialValues(initialCarrierValues);
          setSnackbarSeverity("success");
          setSnackbarOpen(true);
  ...
  const rows =
    data?.carriers.map((carrier: CarrierFormBE) => {
      ...
      const laneDetails = laneIds.map((id) => getLaneDetails(id)).join("\n");
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={6} sm={3} md={2}>
                    <FormControl
                      fullWidth
                      size="small"
                      error={touched.pricingUnit && Boolean(errors.pricingUnit)}
                    >
