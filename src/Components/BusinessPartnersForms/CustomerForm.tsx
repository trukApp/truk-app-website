...
const CustomerForm: React.FC = () => {
  ...
  const [updateRecordData, setUpdateRecordData] = useState<Customer | null>(
    null,
  );
  const [updateRecordId, setUpdateRecordId] = useState(0);
  const [updatePartnerDetails, { isLoading: editCustomerLoading }] =
    useEditBusinessPartnerMutation();
  const [customerRegistration, { isLoading: postCustomerLoading }] =
    useCustomerRegistrationMutation();
  const [deleteBusinessPartner, { isLoading: deleteCustomerLoading }] =
    useDeleteBusinessPartnerMutation();
  ...
  const { data: locationsData, error: getLocationsError } =
    useGetLocationMasterQuery([]);
  ...
  const displayLocations =
    searchKey === "" ? getAllLocations : filteredLocations?.results || [];
  ...
  const rows = customersData.map((item: Customer) => {
    return {
      ...
      bill_to_party:
        getCustomerDetails(item.partner_functions?.bill_to_party) || "",
      ship_to_party:
        getCustomerDetails(item.partner_functions?.ship_to_party) || "",
      sold_to_party:
        getCustomerDetails(item.partner_functions?.sold_to_party) || "",
    };
  });
  ...
  const handleDelete = async (rowData: Customer) => {
    ...
    try {
      ...
      if (response.data.deleted_record) {
        setSnackbarMessage(
          `Customer ID ${response.data.deleted_record} deleted successfully!`,
        );
  ...
  const handleCustomerSubmit = async (
    values: typeof initialCustomerValues,
    ...
    try {
      ...
      if (updateRecord) {
        const response = await updatePartnerDetails({
          body: editBody,
          partnerId: updateRecordId,
        }).unwrap();
        if (response?.updated_record) {
          setSnackbarMessage(
            `Customer ID ${response.updated_record} updated successfully!`,
          );
          resetForm();
          setShowForm(false);
        ...
        if (response?.created_records) {
          setSnackbarMessage(
            `Customer ID ${response.created_records[0]} created successfully!`,
          );
          setFormInitialValues(initialCustomerValues);
          setShowForm(false);
          setUpdateRecord(false);
  ...
  return (
    <Grid>
      <Backdrop
        ...
        open={
          postCustomerLoading || editCustomerLoading || deleteCustomerLoading
        }
      >
          ...
          >
            {({
              ...
              <Form>
                ...
                <Grid container spacing={2} style={{ marginBottom: "30px" }}>
                  <Grid item xs={12} sm={6} md={2.4}>
                    <TextField
                      ...
                      error={
                        touched?.locationOfSource &&
                        Boolean(errors?.locationOfSource)
                      }
                      helperText={
                        touched?.locationOfSource &&
                        typeof errors?.locationOfSource === "string"
                    ...
                    <FormControlLabel
                      control={
                        <Checkbox
                          ...
                          onChange={(e) =>
                            setFieldValue("podRelevant", e.target.checked)
                          }
                        />
                      }
                      label="POD Relevant"
                ...
                <Grid container spacing={2} style={{ marginBottom: "30px" }}>
                  {["shipToParty", "soldToParty", "billToParty"].map(
                    (field) => (
                      <Grid item xs={12} sm={6} md={2.4} key={field}>
                        <Autocomplete
                          options={customersData}
                          getOptionLabel={(option) =>
                            `${option.customer_id} - ${option.name}`
                          }
                          renderOption={(props, option) => (
                            <li {...props}>
                              {option.customer_id} - {option.name},{" "}
                              {option.location_city}, {option.location_state},{" "}
                              {option.location_country},{" "}
                              {option.location_pincode}
                            </li>
                          )}
                          renderInput={(params) => (
                            <TextField
