...
const TransportationLanes = () => {
  ...
  const [postLane, { isLoading: postLaneLoading }] =
    usePostLaneMasterMutation();
  const [editLane, { isLoading: editLaneLoading }] =
    useEditLaneMasterMutation();
  const [deleteLane, { isLoading: deleteLaneLoading }] =
    useDeleteLaneMasterMutation();
  ...
  const {
    data: destinationFilteredLocations,
    isLoading: filteredDestinationLoading,
  } = useGetFilteredLocationsQuery(
    searchKeyDestination.length >= 3 ? searchKeyDestination : null,
    { skip: searchKeyDestination.length < 3 },
  );
  const displayLocations = searchKey
    ? filteredLocations?.results || []
    : getAllLocations;
  const displayLocationsDest = searchKeyDestination
    ? destinationFilteredLocations?.results || []
    : getAllLocations;
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      if (
        wrapperRefDest.current &&
        !wrapperRefDest.current.contains(event.target as Node)
      ) {
    ...
    try {
      ...
      if (isEditing && editRow) {
        ...
        if (response.updated_record) {
          ...
          setIsEditing(false);
          setSnackbarMessage(
            `Lane ID ${response.updated_record} updated successfully!`,
          );
        ...
        if (response.created_records) {
          formik.resetForm();
          setSnackbarMessage(
            `Lane ID ${response.created_records[0]} created successfully!`,
          );
          setSnackbarSeverity("success");
          setSnackbarOpen(true);
          setShowForm(false);
  ...
  const handleDelete = async (row: LaneDetails) => {
    ...
    try {
      ...
      if (response.data.deleted_record) {
        setSnackbarMessage(
          `Lane ID ${response.data.deleted_record} deleted successfully!`,
        );
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      transportDistance: Yup.string().required(
        "Transport Distance is required",
      ),
      transportDuration: Yup.string().required(
        "Transport Duration is required",
      ),
  ...
  useEffect(() => {
    if (editRow) {
      const sourceLocId = editRow?.sourceLocationId
        ? (editRow.sourceLocationId.split(", ")[0] ?? "")
        : "";
      const destiLocId = editRow?.destinationLocationId
        ? (editRow.destinationLocationId.split(", ")[0] ?? "")
        : "";
  ...
  const columns: GridColDef[] = [
    ...
    { field: "transportEndDate", headerName: "Transport End Date", width: 180 },
    {
      ...
      headerName: "Transport Distance",
      width: 160,
    },
    {
      field: "transportDuration",
      headerName: "Transport Duration",
      width: 160,
    },
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} sm={6} md={2.4}>
                  <TextField
                    ...
                    value={searchKey}
                    error={
                      formik.touched?.sourceLocationId &&
                      Boolean(formik.errors?.sourceLocationId)
                    }
                    helperText={
                      formik.touched?.sourceLocationId &&
                      typeof formik.errors?.sourceLocationId === "string"
                ...
                <Grid item xs={12} sm={6} md={2.4}>
                  <TextField
                    ...
                    helperText={
                      formik.touched?.destinationLocationId &&
                      typeof formik.errors?.destinationLocationId === "string"
                        ? formik.errors.destinationLocationId
                        : ""
                    }
                    InputProps={{
                      endAdornment: filteredDestinationLoading ? (
                        <CircularProgress size={20} />
                      ) : null,
                    }}
                  />
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} sm={6} md={2.4}>
                  <TextField
                    ...
                    onBlur={formik.handleBlur}
                    error={
                      formik.touched.transportStartDate &&
                      Boolean(formik.errors.transportStartDate)
                    }
                    helperText={
                      formik.touched.transportStartDate &&
                      formik.errors.transportStartDate
                    }
                ...
                <Grid item xs={12} sm={6} md={2.4}>
                  <TextField
                    ...
                    onChange={formik.handleChange}
                    error={
                      formik.touched.transportEndDate &&
                      Boolean(formik.errors.transportEndDate)
                    }
                    helperText={
                      formik.touched.transportEndDate &&
                      formik.errors.transportEndDate
                    }
                    size="small"
                    InputLabelProps={{ shrink: true }}
                    inputProps={{ min: new Date().toISOString().split("T")[0] }}
                ...
                <Grid item xs={12} sm={6} md={1.6}>
                  <TextField
                    ...
                    onBlur={formik.handleBlur}
                    error={
                      formik.touched.transportDistance &&
                      Boolean(formik.errors.transportDistance)
                    }
                    helperText={
                      formik.touched.transportDistance &&
                      formik.errors.transportDistance
                    }
                    size="small"
                  />
                </Grid>
                ...
                <Grid item xs={12} sm={6} md={1.6}>
                  <TextField
                    ...
                    onBlur={formik.handleBlur}
                    error={
                      formik.touched.transportDuration &&
                      Boolean(formik.errors.transportDuration)
                    }
                    helperText={
                      formik.touched.transportDuration &&
                      formik.errors.transportDuration
                    }
                    size="small"
                  />
                </Grid>
