...
const DeviceMaster: React.FC = () => {
	...
	const { data, error, isLoading, refetch } = useGetDeviceMasterQuery({
		page: paginationModel.page + 1,
		limit: paginationModel.pageSize,
	});
	const [postDevice, { isLoading: postDeviceLoading, isSuccess: postSuccess }] =
		usePostDeviceMasterMutation();
	const [editDevice, { isLoading: editDeviceLoading, isSuccess: editSuccess }] =
		useEditDeviceMasterMutation();
	const [
		deleteDevice,
		{ isLoading: deleteDeviceLoading, isSuccess: deleteSuccess },
	] = useDeleteDeviceMasterMutation();
	...
	const getAllCarriers =
		carriersData?.carriers.length > 0 ? carriersData?.carriers : [];
	...
	const getCarriersDetails = (carrier_ID: string) => {
		const carrier = getAllCarriers.find(
			(carr: CarrierFormBE) => carr.carrier_ID === carrier_ID
		);
		if (!carrier) return "NA";
		const { carrier_name, carrier_address, carrier_loc_of_operation } = carrier;
		const locationDetails =
			carrier_loc_of_operation && carrier_loc_of_operation.length > 0
		...
		try {
			...
			if (isEditing && editRow) {
				...
				if (response?.updated_record) {
					setSnackbarMessage(
						`Device ID ${response.updated_record} updated successfully!`
					);
					formik.resetForm();
					setIsEditing(false);
				...
				if (response?.created_records) {
					setSnackbarMessage(
						`Device ID ${response.created_records[0]} created successfully!`
					);
					formik.resetForm();
					setSnackbarSeverity("success");
					setSnackbarOpen(true);
	...
	const handleDelete = async (row: DeviceMasterValues) => {
		...
		try {
			...
			if (response.data.deleted_record) {
				setSnackbarMessage(
					`Device ID ${response.data.deleted_record} deleted successfully!`
				);
	...
	useEffect(() => {
		if (editRow) {
			...
			const locId = editRow?.locationId
				...
				: "";
			const carrId = editRow?.carrierId
				? editRow.carrierId.split(", ")[0] ?? ""
				: "";
						...
						<Grid container spacing={2}>
							...
							<Grid item xs={12} sm={6} md={2.4}>
								<TextField
									...
									onBlur={formik.handleBlur}
									error={
										formik.touched.deviceUID && Boolean(formik.errors.deviceUID)
									}
									helperText={
										formik.touched.deviceUID && formik.errors.deviceUID
									}
								/>
							</Grid>
							...
							<Grid item xs={12} sm={6} md={2.4}>
								<TextField
									...
									onBlur={formik.handleBlur}
									error={
										formik.touched.simImeiNumber &&
										Boolean(formik.errors.simImeiNumber)
									}
									helperText={
										formik.touched.simImeiNumber && formik.errors.simImeiNumber
									}
								/>
							</Grid>
												...
												>
													...
													>
														<Typography noWrap>
															...
															{carrier?.carrier_address}, {carrier?.carrier_ID}
														</Typography>
													</Tooltip>
												</MenuItem>
											))
