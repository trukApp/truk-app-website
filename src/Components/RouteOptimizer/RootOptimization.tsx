...
const RootOptimization: React.FC<Props> = ({
  rootOptimization,
  onUpdateSampledPoints,
  onBack,
  onSaveDraft,
  ...
  const [returnRoute, setReturnRoute] =
  ...
  const fetchDirections = useCallback(async () => {
    ...
    for (const route of vehicleRoutes) {
      try {
        const response = await new Promise<google.maps.DirectionsResult | null>(
          (resolve) => {
