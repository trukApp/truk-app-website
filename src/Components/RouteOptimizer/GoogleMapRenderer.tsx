...
const GoogleMapRenderer: React.FC<GoogleMapRendererProps> = ({
  ...
  console.log(
    "Rendering GoogleMapRenderer with selectedVehicleData:",
    selectedVehicleData,
  );
  const defaultCenter = {
  ...
  const suggestedRouteRef = useRef<{
    totalDistance: number;
    totalDuration: number;
  } | null>(null);
  ...
  //   useEffect(() => {
  //     onRouteModified?.(avoidTolls || avoidHighways || showWeather);
  //   }, [avoidTolls, avoidHighways, showWeather, onRouteModified]);
  useEffect(() => {
    onRouteModified?.(avoidTolls || avoidHighways || showWeather);

    onRouteSettingsChange?.({
      is_weather: showWeather,
      avoid_tolls: avoidTolls,
      avoid_highways: avoidHighways,
    });
  }, [avoidTolls, avoidHighways, showWeather]);
  ...
  const calculateRoutes = async () => {
    if (!isLoaded || !window.google || !selectedVehicleData?.route?.length)
      return;
    ...
    if (alternateRoutes.length > 0) {
      ...
      altDistance = altLegs.reduce(
        (sum, l) => sum + (l.distance?.value ?? 0),
        0,
      );
      altDuration = altLegs.reduce(
        ...
        >
          ...
          {selectedVehicleData?.route?.length > 0 && (
            <>
              ...
              <Marker
                position={{
                  lat: selectedVehicleData.route[
                    selectedVehicleData.route.length - 1
                  ].end.latitude,
                  lng: selectedVehicleData.route[
                    selectedVehicleData.route.length - 1
                  ].end.longitude,
                }}
                label={{
          ...
          {selectedVehicleData?.loadArrangement?.map(
            (stop: any, index: number) => {
              ...
              const isLastStop =
                index === selectedVehicleData.loadArrangement.length - 1;
                      ...
                      <div style={{ minWidth: 200 }}>
                        ...
                        <div style={{ marginTop: 6 }}>
                          ...
                          <div>
                            ...
                            {formatDistance(summary?.distanceValue)}
                          </div>
                          <div>
                            <strong>ETA:</strong>{" "}
                            {formatDurationSeconds(summary?.durationValue)}
                          </div>
                          {showWeather && weather && (
                            <div
                              style={{
          ...
          >
            {[
              ...
              // { label: 'Traffic', state: showTraffic, setter: setShowTraffic },
              {
                label: "Avoid Tolls",
                state: avoidTolls,
                setter: setAvoidTolls,
              },
              {
                label: "Avoid Highways",
                state: avoidHighways,
                setter: setAvoidHighways,
              },
            ...
            <tbody>
              {selectedVehicleData.loadArrangement.map(
                (stop: any, index: number) => {
                  ...
                  const delaySeconds =
                    (summary.durationTrafficValue ?? summary.durationValue) -
                  ...
                  return (
                    <tr key={index} style={{ borderBottom: "1px solid #eee" }}>
                      ...
                      <td>
                        {formatDurationSeconds(summary.durationTrafficValue)}
                      </td>
                      <td style={{ color, fontWeight: 600 }}>
                        {delaySeconds > 0
                          ? `+${formatDurationSeconds(delaySeconds)}`
                          : "—"}
                      </td>
