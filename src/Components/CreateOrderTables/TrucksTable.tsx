...
// //                     position: [
// //                       b.position[0] ?? 0,
// //                       noStack ? 0 : b.position[1] ?? 0, // enforce floor if no stacking
// //                       b.position[2] ?? 0,
// //                     ],
// //                   }));
...
//   /** Robust normalizeBlocks - recursively flattens all boxes */
//   const normalizeBlocks = (
//     boxPlacements:
//       | {
//         pkg_ID: string;
...
//           const allowedLayers = truck.truckCapacity?.allowedLayers ?? 1;
//           const allowedByHeight =
//             truck.truckCapacity?.allowedByHeight ?? truck.truckCapacity?.maxLayersByHeight;
//           const allowedBySF = truck.truckCapacity?.allowedBySF;
//           const layersUsed = truck.truckCapacity?.layersUsed;
...
//                   // Force Y=0 for no stacking
//                   const safeBlocks: PackageBlock[] = blocks.map((b) => ({
//                     ...b,
//                     position: [b.position[0] ?? 0, noStack ? 0 : b.position[1] ?? 0, b.position[2] ?? 0],
//                   }));
                    ...
                    {openTruck.loadArrangement.map((s) => (
                      <li key={s.stop}>
                        ...
                        <Button
                          ...
                          onClick={() => {
                            setExpandedStops((prev) =>
                              prev.includes(s.stop) ? prev.filter((x) => x !== s.stop) : [...prev, s.stop]
                            );
                          }}
                          sx={{ ml: 1 }}
                        >
                          ...
                          <Box sx={{ pl: 2, mt: 1 }}>
                            {s.packages.map((pkgID) => {
                              const details =
                                openTruck.productLegend
                                  ?.map((prod) => {
                                    const found = prod.byPackage.find((bp) => bp.pack_ID === pkgID);
                                    return found
                                      ? { prod_ID: prod.prod_ID, prodName: getProductName(prod.prod_ID), qty: found.qty, color: found.color || prod.color }
                                      : null;
                                  })
                                  .filter(Boolean) || [];
                              return (
