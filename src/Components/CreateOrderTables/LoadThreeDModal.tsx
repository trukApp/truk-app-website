...
interface PackageBlock {
	...
	color: string;
	dimensions: [number, number, number]; // [L, H, W]
	position: [number, number, number];   // [xFront, y, z] — BE gives x from the FRONT wall
	quantity: number;
}
...
interface TruckSceneProps {
	vehicleDimensions: {
		length: number; // interior cargo length (m)
		width: number;  // interior cargo width (m)
		height: number; // interior cargo height (m)
	};
	packageBlocks: PackageBlock[];
	truckCapacity?: {
		allowedLayers: number; // <=1 => no stacking
...
const TruckScene: React.FC<TruckSceneProps> = ({
	...
	// If allowedLayers <= 1, clamp all Y to ground
	const singleLayer = (truckCapacity?.allowedLayers ?? 1) <= 1;

	// Convert BE x (from FRONT wall) -> renderer x (from BACK wall)
	const xFrontToBack = (xFront: number, L: number) => length - xFront - L;
		...
		<div style={{ display: 'flex', gap: '2rem' }}>
			<div style={{ flex: 2 }}>
				...
				>
					...
					{/* Boxes — use BE coords, but convert x to back-origin and clamp y if single layer */}
					{packageBlocks.map((block, index) => {
						const [L, H, W] = block.dimensions;
						const [xFront, yBE, z] = block.position;
						const xBack = xFrontToBack(xFront, L);
						...
						return (
							<mesh
								key={`${block.pkg_ID}-${index}`}
								position={[
									cabinLength + xBack + L / 2,   // ✅ converted X
									steelPlateY + y0 + H / 2,      // ✅ Y clamped when single layer
									z + W / 2,
								]}
								castShadow
								receiveShadow
					...
					{/* Debug planes (optional): front=red, rear=green */}
					{false && (
						<>
							<mesh position={[cabinLength + 0.01, steelPlateY + 0.01, width / 2]}>
								<boxGeometry args={[0.02, height - steelPlateY - 0.02, width - 0.02]} />
								<meshBasicMaterial color="red" transparent opacity={0.25} />
							</mesh>
							<mesh position={[cabinLength + length - 0.01, steelPlateY + 0.01, width / 2]}>
								<boxGeometry args={[0.02, height - steelPlateY - 0.02, width - 0.02]} />
