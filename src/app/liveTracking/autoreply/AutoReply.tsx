  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch(
        `https://gpsvtsprobend.vamosys.com/getVehicleHistory?vehicleId=${vehicleId}&fromDateUTC=${fromDateUTC}&toDateUTC=${toDateUTC}&interval=-1&userId=${userId}`,
      );
      const data = await response.json();
      setVehicleType(data?.vehicleType);

      if (response.ok) {
        const locations = data?.vehicleLocations?.map(
        ...
        const deviatedLocations =
          data?.vehicleLocations?.map(
            (loc: { latitude: string; longitude: string }) => ({
    ...
    return (
      ...
      >
        <Typography variant="h6" color="error">
          No vehicle locations available. Please check the vehicle status or try
          again later.
        </Typography>
      </Grid>
    );
