...
import { Backdrop, CircularProgress, Grid, Paper, Typography, Box, Button, Tooltip, Dialog, DialogContent, DialogActions, IconButton, DialogTitle } from "@mui/material";
...
const OrderDetailedOverview: React.FC = () => {
    ...
    const pdfRef = useRef<HTMLDivElement>(null);
      const [editOrder, { isLoading: confirmOrderLoading }] = useEditOrderMutation();
      ...
      >
        {openPreview.url.endsWith(".pdf") ? (
          ...
          />
            ...
            <Backdrop sx={{ color: "#fff", zIndex: (theme) => theme.zIndex.drawer + 1 }} open={isLoading || confirmOrderLoading}>
            ...
            {orderData && (
                    <Paper sx={{ p: 3, mb: 3, marginLeft: isGeneratingPDF ? '30px' : '2px' }}>
                        <Grid sx={{display:'flex', flexDirection:'row', justifyContent:'space-between'}}>
                            ...
                            {orderData?.order_status === 'assignment pending' && !isGeneratingPDF && (
                            <Grid sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'flex-end', alignSelf: 'flex-end' }}>
                            <Tooltip title="Download the full order details as a PDF" arrow>
                                <Button 
                                type="button"
                                sx={{
                    ...
                    <Grid container spacing={1}  >
                                ...
                                <Grid item xs={12} md={3} key={index}>
                                    ...
                                    {key}
                                        ...
                                        <Grid spacing={2} container
                                            sx={{ cursor: "pointer" ,display:'flex', flexDirection:'column',marginTop:'1px', marginLeft:'1px' }}
                        ...
                        <Grid sx={{display:'flex', flexDirection:'row',justifySelf:'flex-end' ,alignSelf:'flex-end'}}>
                            <Button sx={{textDecoration:'underline'}}  onClick={handleOpenDialog} >
                            Add Documents
                        </Button>
                        </Grid>
