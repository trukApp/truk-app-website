...
const ProductMasterPage: React.FC<ProductMasterProps> = ({
  productsFromServer,
}) => {
  ...
  const { data: productsDataFromClient, isLoading } = useGetAllProductsQuery({
  ...
  const { data: packagesData, isLoading: isPackageLoading } =
    useGetPackageMasterQuery({});
  const [createNewProduct, { isLoading: createLoading }] =
    useCreateProductMutation();
  const [deleteProduct, { isLoading: deleteProductLoading }] =
    useDeleteProductMutation();
  const [updateProductDetails, { isLoading: updateProductLoading }] =
    useEditProductMutation();
  ...
  const validationSchema = Yup.object({
    ...
    chargeableWeightUnit: Yup.string().required(
      "Chargeable weight unit is required",
    ),
  ...
  const getPackageDetails = (pack_ID: string) => {
    const packageData = getAllPackages.find(
      (pack: Package) => pack.pac_ID === pack_ID,
    );
    return packageData
  ...
  const mapRowToInitialValues = (selectedProduct: Product) => {
    const locId = selectedProduct?.locationId
      ? (selectedProduct?.locationId.split(", ")[0] ?? "")
      : "";
    const packIdd = selectedProduct?.packagingType
      ? (selectedProduct?.packagingType.split(", ").at(-1) ?? "")
      : "";
  ...
  const handleDelete = async (row: Product) => {
    ...
    try {
      ...
      if (response.data.deleted_record) {
        setSnackbarMessage(
          `Product ID ${response.data.deleted_record} deleted successfully!`,
        );
  ...
  const columns: GridColDef[] = [
    ...
    {
      field: "chargeablwWeightUom",
      headerName: "Chargeable Weight Unit",
      width: 150,
    },
  ...
  const handleSubmit = async (
    values: typeof productFormInitialValues,
    ...
    try {
      ...
      if (updateRecord) {
        const response = await updateProductDetails({
          ...
          productId: updateRecordId,
        }).unwrap();
        if (response?.updated_record) {
          setSnackbarMessage(
            `Product ID ${response.updated_record} updated successfully!`,
          );
          resetForm();
        ...
        if (response?.created_records) {
          setSnackbarMessage(
            `Product ID ${response.created_records[0]} created successfully!`,
          );
          resetForm();
          setShowForm(false);
          setUpdateRecord(false);
    ...
    <>
      ...
      <Grid sx={{ margin: { xs: "0px", md: "0px 30px" } }}>
        ...
        <Typography variant="body2" color="text.secondary">
          Add and Manage Products Used in Your Packaging and Logistics
                  ...
                  <Grid container spacing={2}>
                    ...
                    <Grid item xs={12} sm={6} md={2.4}>
                      <TextField
                        ...
                        onBlur={handleBlur}
                        error={
                          touched.chargeableWeight &&
                          Boolean(errors.chargeableWeight)
                        }
                        helperText={
                          touched.chargeableWeight && errors.chargeableWeight
                        }
                      />
                    </Grid>
                    <Grid item xs={12} sm={6} md={2.4}>
                  ...
                  <Grid container spacing={2}>
                    ...
                    <Grid item xs={12} sm={6} md={2.4}>
                      <TextField
                        ...
                        inputProps={{ min: 0 }}
                        error={
                          touched.stackingFactor &&
                          Boolean(errors.stackingFactor)
                        }
                        helperText={
                          touched.stackingFactor && errors.stackingFactor
                        }
                      />{" "}
                      <Typography
                        variant="caption"
                      ...
                      >
                        Stacking factor (0-infinite, 1-no stacking, &gt;1 is the
                        no of layers/stack)
                      </Typography>
                    </Grid>
                  ...
                  <Grid container spacing={2}>
                    ...
                    <Grid item xs={12} sm={6} md={2.4}>
                      <FormControl
                        ...
                        size="small"
                        error={
                          touched.packagingType && Boolean(errors.packagingType)
                        }
                      >
                          ...
                          {isPackageLoading ? (
                            ...
                            getAllPackages?.map((packages: Package) => (
                              ...
                              >
                                ...
                                >
                                  <span>
                                    {packages.packaging_type_name},{" "}
                                    {packages.pack_length}*{packages.pack_width}
                                    *{packages.pack_height}{" "}
                                    {packages.dimensions_uom},{" "}
                                    {packages.pac_ID}{" "}
                                  </span>
                                </Tooltip>
                              </MenuItem>
