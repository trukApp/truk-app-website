...
import { Formik, Form, FieldArray, FormikHelpers, useFormikContext } from 'formik';
import { useRouter } from 'next/navigation';
...
const shipToInitialValues = {
    ...shipFromInitialValues,
    destination_radius: '0',
    destination_radius_unit: 'm',
};

const billToInitialValues = { ...shipFromInitialValues };
...
const pickupDropoffInitialValues = {
    pickupDateTime: '',
    dropoffDateTime: '',
    notes: '',
};

const singlePackageInitialValues = {
    shipFrom: shipFromInitialValues,
    shipTo: shipToInitialValues,
    billTo: billToInitialValues,
    packageDetails: packageDetailsInitialValues,
    additionalInfo: additionalInfoInitialValues,
    pickupDropoff: pickupDropoffInitialValues,
};

/* ================= ADD PACKAGE BUTTON ================= */

const AddPackageButton = ({ onAdd }: { onAdd: () => void }) => {
    const { values, errors } = useFormikContext<CreatePackageFormValues>();
    const lastIndex = values.packages.length - 1;

    const isLastPackageValid =
        !errors.packages || !errors.packages[lastIndex];

    return (
        <Box textAlign="center" mb={4}>
            <Button
                variant="outlined"
                disabled={!isLastPackageValid}
                onClick={onAdd}
            >
                Add Another Package
            </Button>
...
const CreatePackage = () => {
    ...
    const [createPackageOrder, { isLoading }] =
    ...
    const handleSubmit = async (
        ...
        helpers: FormikHelpers<CreatePackageFormValues>
    ) => {
        const body = {
            packages: values.packages.map((pkg) => ({
                ship_from: pkg.shipFrom.locationId?.split(',')[0],
                ship_to: pkg.shipTo.locationId?.split(',')[0],
                destination_radius: `${pkg.shipTo.destination_radius}${pkg.shipTo.destination_radius_unit}`,
                bill_to: pkg.billTo.locationId?.split(',')[0],
                product_ID: pkg.packageDetails.map((p: any) => ({
                    prod_ID: p.productId,
                    quantity: p.quantity,
                    package_info: p.packagingType,
                })),
                return_label: pkg.additionalInfo.returnLabel ? 1 : 0,
                additional_info: {
                    reference_id: pkg.additionalInfo.referenceId,
                    invoice: pkg.additionalInfo.invoiceNumber,
                    po_number: pkg.additionalInfo.poNumber,
                    sales_order_number: pkg.additionalInfo.salesOrderNumber,
                    department: pkg.additionalInfo.department,
                    delivery_type: pkg.additionalInfo.deliveryType,
                    delivery_mode: pkg.additionalInfo.deliveryMode,
                    value_of_goods: pkg.additionalInfo.valueOfGoods,
                    eWay_bill_file: pkg.additionalInfo.eWayBillFile,
                    stn_Do_number: pkg.additionalInfo.stnDoNumber,
                    attachment: pkg.additionalInfo.file,
                },
                pickup_date_time: pkg.pickupDropoff.pickupDateTime,
                dropoff_date_time: pkg.pickupDropoff.dropoffDateTime,
            })),
        };

        try {
            const res = await createPackageOrder(body).unwrap();
            setSnackbarMessage(
    ...
    return (
        <>
            <Backdrop open={isLoading} sx={{ zIndex: 1300 }}>
                <CircularProgress color="inherit" />
            </Backdrop>
            ...
            <Dialog open={modalOpen} onClose={() => setModalOpen(false)}>
                <DialogTitle>Packages Created</DialogTitle>
                <DialogContent>
                    <Typography>
                        Packages created successfully. What would you like to do next?
                    </Typography>
                </DialogContent>
                <DialogActions>
                    ...
                    <Button onClick={() => router.push('/createorder')}>
                        Go to Create Order
                    </Button>
                </DialogActions>
            </Dialog>

            <Box px={isMobile ? 2 : 4} pb={2}>
            ...
            <Formik<CreatePackageFormValues>
                initialValues={{ packages: [singlePackageInitialValues] }}
                validationSchema={createPackageValidationSchema}
                onSubmit={handleSubmit}
            >
                <Form>
                    <FieldArray name="packages">
                        {({ push }) => (
                            <>
                                ...
                                <PackageForms />
                                <AddPackageButton
                                    onAdd={() => push(singlePackageInitialValues)}
                                />
                            </>
                        )}
                    </FieldArray>
...
const PackageForms = () => {
    const { values } = useFormikContext<CreatePackageFormValues>();

    return (
        <>
            {values.packages.map((_, index) => (
                <Box key={index} mb={4} p={2} border="1px solid #ddd">
                    <Typography variant="h6" mb={2}>
                        Package {index + 1}
                    </Typography>

                    <ShipFrom index={index} />
                    <ShipTo index={index} />
                    <BillTo index={index} />
                    <PackageDetails index={index} />
                    <AdditionalInformation index={index} />
                    <PickupDropoff index={index} />
                </Box>
            ))}
        </>
    );
