...
const TrackingPage = () => {
  ...
  const [trackedShipment, setTrackedShipment] = useState<ShipmentRow | null>(
    null,
  );
  ...
  const shipments: ShipmentRow[] = useMemo(() => {
    return (
      data?.shipments?.map((item: Shipment) => ({
        ...
        path: item.path || [],
        vehicle:
          item.assignment?.assigned_vehicle_data?.[0]?.self_vehicle_num ||
          "N/A",
  ...
  return (
    <Grid sx={{ p: 2, minHeight: "100%", bgcolor: "#f4f6f8" }}>
      <Typography
        variant="h5"
        fontWeight="bold"
        color="primary.main"
            ...
            <Grid item xs={12} md={2.4} key={i}>
              <Grid
                ...
                sx={{
                  p: 2,
                  borderRadius: 3,
                  bgcolor: active ? theme.palette.primary.main : "#ffffff",
      ...
      >
                          ...
                          <React.Fragment key={route.id}>
                            ...
                            {route.current_position && (
                              <Marker
                                opacity={isSelected ? 1 : 0.2}
                                position={{
                                  lat: Number(route.current_position.latitude),
                                  lng: Number(route.current_position.longitude),
                                }}
        ...
        <Grid item xs={12} md={3}>
          ...
          >
            {/* HEADER */}
            <Grid display="flex" alignItems="center" gap={1} mb={1.5}>
              <WarningAmberIcon
                sx={{
                  color: "#f59e0b",
                  fontSize: 20,
            ...
            {/* ALERT CARDS */}
            <Grid sx={{ overflowY: "auto", flex: 1, pr: 0.5 }}>
              {filteredAlerts.length > 0 ? (
                filteredAlerts.map((a: AlertItem, i: number) => (
                  ...
                  >
                    ...
                    {/* MESSAGE */}
                    <Typography fontSize={12.5} color="#111827">
                      {a.text}
                    </Typography>
                  </Grid>
                ))
              ...
              >
                ...
                >
                  <Grid display="flex" alignItems="center" gap={0.5}>
                    ...
                    <Typography fontSize={11} fontWeight={600} color="#6b7280">
                      Control Tower
                    </Typography>
                  </Grid>
      ...
      >
        ...
        >
          <Grid display="flex" alignItems="center" gap={1}>
            ...
            <Typography color="#111827" fontWeight={700} fontSize={18}>
              Active Shipments
            </Typography>
          </Grid>
        ...
        >
          <Grid sx={{ minWidth: 1450 }}>
            ...
            {/* TABLE ROWS */}
            {filteredShipments.length > 0 ? (
              filteredShipments.map((row: ShipmentRow, i: number) => (
                <Grid
                  key={i}
                  display="grid"
                  gridTemplateColumns="1.4fr 2.5fr 1.4fr 1.1fr 1.5fr 1.8fr 1fr 1.3fr 1.2fr 0.8fr"
              ...
              <Grid
                ...
                height="220px"
              >
                <Typography color="#6b7280" fontSize={15} fontWeight={600}>
                  No orders available
                </Typography>
              </Grid>
            )}
        ...
        <DialogContent sx={{ p: 0, position: "relative", height: "100%" }}>
          ...
          {isLoaded && trackedShipment && (
            ...
            >
              ...
              {/* END POINT */}
              {trackedShipment.path?.[trackedShipment.path.length - 1] && (
                <Marker
                  position={
                    trackedShipment.path[trackedShipment.path.length - 1]
                  }
